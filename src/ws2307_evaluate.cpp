...
enum _ADCVALS {
  ...
  UFLTFWDPEAK,
  UDRVFWDPEAK,
  // und die Indizes fuer die berechneten Werte
  CALC_POWER,
  CALC_EFF,
  MFREQUENCY,
  NUMADCVALS
...
void print32(char *ps, long v)
{
  ...
  v = labs(v);
  sprintf(ps, "%c%ld.%1ld", sign, v / 10, v - (v / 10) * 10);
}
...
void evaluate_DSP7data0(unsigned char *ws_rxdata)
{
  ...
  if (len > 0)
  {
    ...
    prxdata = ws_rxdata + 6; // ohne Header und Laenge
    ...
    if (temp > 0 && temp < 1590)
      sprintf(t_vals[TEMPERATURE1], "%lu.%1lu", temp / 10, temp - (temp / 10) * 10);
    else
      sprintf(t_vals[TEMPERATURE1], "?");

    temp = adc_realvals[UTEMP2];
    if (temp > 0 && temp < 1590)
      sprintf(t_vals[TEMPERATURE2], "%lu.%1lu", temp / 10, temp - (temp / 10) * 10);
    else
...
// die Laenge ist stm32_cfglen, Inhalt in stm32_config
...
int len;
  ...
  // ermittle zunaechst die Laenge
  len = BCD_to_HEX(p[4*2],p[4*2+1]);
  len <<= 8;
  len |= BCD_to_HEX(p[5*2],p[5*2+1]);
  len += 8; // header, laenge und crc16 dazu
