...
void writeEEPROM()
{
int adr=10; // die ersten 10 lasse frei fuer Magic, Chksum
  ...
  eeprom_printf((char *)"EEPROM geschrieben. Reserve %d Bytes",reserved_eeprom_size-adr);
}
...
char ok=1;
  ...
  {
    // init some arrays
    eeprom_printf((char *)"EEPROM Neustart mit Defaultwerten");
    ok=0;
  }
  ee_endread();
  led_access_eeprom(0);
...
unsigned long ee_magic=0x12345678;
...
unsigned long rchksum=0, uist;
  ...
  EEPROM_readAnything(0,ee_magic);

  if(ee_magic == MAGIC && uist == rchksum) return 1;
